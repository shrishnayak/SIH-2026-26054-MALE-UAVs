...
const RadialGauge = ({ value, min, max, label, unit, criticalLow, warningLow, warningHigh, criticalHigh, precision = 1 }) => {
  ...
  if ((criticalHigh && value >= criticalHigh) || (criticalLow && value <= criticalLow)) {
    ...
    color = '#E5A93C'; // Tactical Amber
  }

  return (
    <div className="flex flex-col items-center justify-center p-3.5 bg-carbon-900/90 border border-carbon-700 rounded-xl relative overflow-hidden shadow-hud-glass">
      ...
      <div className="relative w-28 h-16 flex items-end justify-center">
        ...
        <div className="absolute bottom-0 text-center">
          ...
          <span className="text-[10px] font-mono text-tactical-amber ml-1">{unit}</span>
        </div>
      </div>
    </div>
  );
...
const DynamicSparkline = ({ data, color = '#2DD4BF', min, max, label, currentVal, unit }) => {
  ...
  return (
    <div className="bg-carbon-900/90 border border-carbon-700 rounded-xl p-3.5 flex flex-col justify-between shadow-hud-glass">
...
export const TelemetryTab = () => {
  ...
  return (
    <div className="flex flex-col gap-4 w-full h-[calc(100vh-140px)] overflow-y-auto pr-1 font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="hud-glass rounded-xl p-4 flex flex-col gap-3">
          <div className="flex items-center justify-between border-b border-tactical-amber/20 pb-2">
            <div className="flex items-center gap-2">
              <Flame className="w-4 h-4 text-tactical-amber" />
          ...
          <div className="grid grid-cols-4 gap-2">
            {engine.egt.map((val, idx) => {
              ...
              const barColor = isCrit ? 'bg-red-500' : isWarn ? 'bg-tactical-amber' : 'bg-cryo-teal';

              return (
                <div key={idx} className="bg-carbon-950 p-2.5 rounded-lg border border-carbon-800 flex flex-col items-center">
                  ...
                  <div className="w-full bg-carbon-900 h-20 rounded-md overflow-hidden flex items-end p-0.5">
                    <div
        ...
        <div className="hud-glass rounded-xl p-4 flex flex-col gap-3">
          <div className="flex items-center justify-between border-b border-cryo-teal/20 pb-2">
            <div className="flex items-center gap-2">
              <Thermometer className="w-4 h-4 text-cryo-teal" />
              <h3 className="font-display font-bold text-xs tracking-wider text-slate-200">
      ...
      <div className="hud-glass rounded-xl p-3.5 flex flex-col gap-2">
        <div className="flex items-center justify-between border-b border-carbon-800 pb-2">
          <div className="flex items-center gap-2">
            <Radio className="w-4 h-4 text-tactical-amber animate-pulse" />
            <span className="font-mono text-xs font-bold text-tactical-amber">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
          {canBusFrames?.map((frame, idx) => (
            <div key={idx} className="bg-carbon-950 p-2.5 rounded-lg border border-carbon-800 font-mono text-xs">
              <div className="flex items-center justify-between text-slate-400 mb-1">
                <span className="text-tactical-amber font-bold">ID: {frame.canId}</span>
                <span className="text-[10px]">DLC: {frame.dlc} BYTES</span>
              </div>
              <div className="text-slate-200 tracking-widest bg-carbon-900 px-2 py-1 rounded text-center border border-carbon-800">
                {frame.rawHex}
              </div>
            </div>
          ))}
